    ...
    <Layout hasTabBar title="나의 캐럿">
      <div className="px-4">
        ...
        {data?.reviews.map(reviews => (
          <div className="mt-12" key={reviews.id}>
            <div className="flex space-x-4 items-center">
              <div className="w-12 h-12 rounded-full bg-slate-500" />
              <div>
                <h4 className="text-sm font-bold text-gray-800">{reviews.createdBy.name}</h4>
                <div className="flex items-center">
                  {[1,2,3,4,5].map(star => (
                    <svg
                    key={star}
                    className={cls("h-5 w-5" , reviews.score >= star ? "text-yellow-400" : "text-gray-400")}
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 20 20"
                    fill="currentColor"
                    aria-hidden="true"
